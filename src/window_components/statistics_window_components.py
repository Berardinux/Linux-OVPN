...
class StatisticsWindowUIComponents:
    def __init__(self):
        ...
        # Define all labels as None so we can fill them later
    ...
    def create_statistics_body_box(self):
        ...
        def add_row(label_text, attr_name):
            ...
            label.get_style_context().add_class("h6")
            label.get_style_context().add_class("color1")
            label.set_halign(Gtk.Align.START)
            self.body_box.pack_start(label, False, False, 0)

            value_label = Gtk.Label()
            value_label.get_style_context().add_class("h6")
            value_label.get_style_context().add_class("color0")
            value_label.set_halign(Gtk.Align.START)
            self.body_box.pack_start(value_label, False, False, 0)
        ...
        add_row("BITS IN ⬇️", "bits_in_value_label")
        add_row("BITS OUT ⬆️", "bits_out_value_label")
        add_row("PACKETS IN ⬇️", "packets_in_value_label")
        add_row("PACKETS OUT ⬆️", "packets_out_value_label")
        add_row("TUN BYTES IN ⬇️", "tun_bytes_in_value_label")
        add_row("TUN BYTES OUT ⬆️", "tun_bytes_out_value_label")
        add_row("TUN PACKETS IN ⬇️", "tun_packets_in_value_label")
        add_row("TUN PACKETS OUT ⬆️", "tun_packets_out_value_label")
    ...
    def update_labels(self):
        ...
        self.packets_in_value_label.set_text(str(stats.get("tun_packets_in", 0)))
        self.packets_out_value_label.set_text(str(stats.get("tun_packets_out", 0)))
    ...
    def start_updating(self, interval_ms=1000):
        """
        Call this to begin periodic updating of statistics.
        """
